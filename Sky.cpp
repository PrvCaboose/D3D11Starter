		 ...
		 vertexShader(vertexShader)
{
	...
	Graphics::Device->CreateRasterizerState(&rast, rasterizer.GetAddressOf());

	D3D11_DEPTH_STENCIL_DESC depth = {};
	depth.DepthEnable = true;
	depth.DepthFunc = D3D11_COMPARISON_LESS_EQUAL;
	Graphics::Device->CreateDepthStencilState(&depth, depthBuffer.GetAddressOf());

	textureSrv = CreateCubemap(right, left, up, down, front, back);
}
...
void Sky::Draw(std::shared_ptr<Camera> camera)
{
	...
	vertexShader->SetMatrix4x4("projMatrix",camera->GetProj());
	vertexShader->CopyAllBufferData();

	pixelShader->SetShaderResourceView("cube",textureSrv);
	pixelShader->SetSamplerState("sample",sampleOptions);
